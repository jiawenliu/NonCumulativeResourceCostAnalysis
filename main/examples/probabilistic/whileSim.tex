\begin{example}[Simple While with Probabilistic Sampling]
    \label{ex:whileSimUniform}
    In this example program $\kw{whilSimUniform(k)}$, the analyst asks in total $k+1$ queries to the mechanism in two phases.
...
$
    ...
    \kw{whileSim(k)} \triangleq \\
           \clabel{ \assign{a}{\euniform(0, 1)}}^{0} ;\\
            \clabel{\assign{j}{k} }^{1} ; \\
            \ewhile ~ \clabel{j > 0}^{2} ~ \edo ~ \\
            \Big(
             \clabel{\assign{x}{\euniform(j, k)} }^{3}  ; \\
            \clabel{\assign{l}{\euniform(x, l)}}^{4}   ;\\    
...
\end{centering}
\end{subfigure}
 \caption{(a) An simple while loop example with sampling 
 from uniform distribution $\kw{whileSimUniform(k)}$ 
 (b) The corresponding execution-based dependency graph.}
\label{fig:whileSimUniform}
\end{figure}
}
\end{example}